...
public class TextText implements WritableComparable<TextText> {
    ...
    public TextText() { // 기본 생성자로, Text 객체를 기본값("")으로 초기화합니다.
        set(new Text(), new Text());
    }

    public TextText(String first, String second) { // String 타입의 값을 받아 Text 객체로 초기화하는 생성자입니다.
        set(new Text(first), new Text(second));
    }

    public TextText(Text first, Text second) { // Text 타입의 값을 받아 초기화하는 생성자입니다
        set(first, second);
    }

    public void set(Text first, Text second) { // first와 second Text 객체를 설정하는 메서드입니다.
        this.first = first;
        this.second = second;
    }
